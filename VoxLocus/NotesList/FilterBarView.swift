...
struct FilterBarView: View {
    ...
    private var categoryChips: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                chip(title: String(localized: "All", comment: "Show notes from all categories"), isSelected: selectedCategory == nil) {
                    withAnimation(.easeInOut(duration: 0.25)) { selectedCategory = nil }
                }
                ForEach(NoteCategory.allCases) { category in
                    chip(title: category.displayName, systemImage: category.systemImage, isSelected: selectedCategory == category) {
